...
abstract class Step3PetRepositoryTest<TRepository : Step3PetRepository> : Step2PetRepositoryTest<TRepository>() {
    @Test
    ...
    fun `the window controls which weigh-ins are counted`() {
        ...
        val pet = repository.create(PetRequest("Fluffy", 10.0), userOneId)
    ...
    fun `the leaderboard counts each actor's weigh-ins`() {
        ...
        val fluffy = repository.create(PetRequest("Fluffy", 10.0), userOneId)
        val sprout = repository.create(PetRequest("Sprout", 5.0), userTwoId)
    ...
    fun `the leaderboard orders actors by weigh-in count descending`() {
        ...
        testClock.setNow(Instant.parse("2026-01-15T00:00:00Z"))
        val pet = repository.create(PetRequest("Fluffy", 10.0), userTwoId)
