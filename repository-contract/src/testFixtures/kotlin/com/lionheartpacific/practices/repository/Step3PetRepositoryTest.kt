...
abstract class Step3PetRepositoryTest<TRepository : Step3PetRepository> : Step2PetRepositoryTest<TRepository>() {
    @Test
    fun `a weigh-in is recorded in the chart`() {
        val userOneId = 10L
        val id = repository.create(PetRequest(name = "Fluffy"), userOneId)
        val recordedAt = Instant.parse("2026-01-01T00:00:00Z")
        testClock.setNow(recordedAt)

        repository.updateWeight(id, 10.0, userOneId)

        expectThat(repository.getWeightChart(id)).isNotNull().get { entries }.containsExactly(
            WeightEntry(recordedAt = recordedAt, weight = 10.0),
        )
    }

    @Test
    fun `weigh-ins appear in the chart in order`() {
        val userOneId = 10L
        val id = repository.create(PetRequest(name = "Fluffy"), userOneId)
